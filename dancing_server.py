...
class L0_dual_jp_move_safe_relate_srv():
    ...
    def L0_point_adding(self, l_rot, r_rot, total_time):
        pass
    ...
    def L0_dual_joint_rot_relate(self, l_rot, r_rot, total_time):
        ...
        current_angles_r = list(self._c.movegroup_rarm_group.get_current_joint_values())
        current_angles_l = list(self._c.movegroup_larm_group.get_current_joint_values())

        print ("current angle r is :")
        ...
        if (self.count<6):
            ...
            print("dt_r is ")
            print(dt_r)
            current_l_jp_position = self.l_jp_position
            current_r_jp_position = self.r_jp_position
            
            print("r_rot is :")
            print(r_rot)
            if (self.count == 0):
                current_l_jp_position = list(current_angles_l)
                current_r_jp_position = list(current_angles_r)
                self.jointcontrol_client_left.add_point(current_angles_l, 0)
                self.jointcontrol_client_right.add_point(current_angles_r, 0)

            print("r_jp before adding")
            print(current_r_jp_position)

            while (du< total_time):
                du += dt_r
                l_jp_position = [current_l_jp_position[i] + l_rot[i] * du / total_time for i in range(len(current_l_jp_position))]
                r_jp_position = [current_r_jp_position[i] + r_rot[i] * du / total_time for i in range(len(current_r_jp_position))]
                self.jointcontrol_client_left.add_point(l_jp_position, du + self.count * total_time)
                self.jointcontrol_client_right.add_point(r_jp_position, du + self.count * total_time)
            print("r_jp after adding")
            print(r_jp_position)
            self.l_jp_position = l_jp_position
            self.r_jp_position = r_jp_position
            self.count += 1
            
        print("   Points added")

        if (self.count == 6):
            print("count is 4, begin to move")
            # print(l_jp_position)
...
class L0_dual_task_move_safe_relate_srv():
    ...
    def L0_dual_tcp_move_relate(self,lmove,rmove,total_time,point_num=10):
        ...
        tmp_all = rospy.wait_for_message("/joint_states", JointState)
        tmp_all_pos = tmp_all.position
        ...
        self.jointcontrol_client_left.add_point(current_angles_r,0)
        self.jointcontrol_client_right.add_point(current_angles_l,0)
    ...
    def execute_cb(self,goal):
        ...
        if(success==True):
            print("hi! dual task move, after if(success==True)")
            self.send_sub_goal(goal)
            print("hi! dual task move, after send_sub_goal(goal)")
            while(not rospy.is_shutdown()):
                r=rospy.Rate(50)
                r.sleep()
                if self._as.is_preempt_requested():
